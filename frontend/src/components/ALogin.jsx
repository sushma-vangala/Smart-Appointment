    ...
    const onSubmitHandler = async (event) => {
        ...
        try {
            ...
            if(state === 'Admin'){
                ...
                const {data} =await axios.post(backendUrl + '/api/providers/login',{email,password})
  ...
  return (
    <form onSubmit={onSubmitHandler} className='flex items-center'>
      <div className='flex flex-col gap-3 m-auto items-start p-8 min-w-[340px] sm:min-w-96 border rounded-xl text-5E5E5E text-sm shadow-lg'>
        <p className='text-2xl font-semibold m-auto'><span className='text-[var(--primary)]'>{state}</span> Login</p>
        ...
        {
            state=== 'Admin' 
            ? <p>Provider Login? <span className='text-[var(--primary)] underline cursor-pointer' onClick={()=>setState('Provider')} >Click here</span></p>
